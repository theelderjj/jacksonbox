...
test.describe("rules intro preview", () => {
  test("renders the standalone intro animation", async ({ page }) => {
    await test.step("Open rules preview", async () => {
      await page.goto("/?preview=rules&game=drawful");
    });

    await test.step("Slide 1 explains drawing", async () => {
    ...
    await test.step("Slide 2 explains fake prompts", async () => {
    ...
    await test.step("Slide 3 explains truth scoring", async () => {
      await page.getByRole("button", { name: "Next" }).click();
